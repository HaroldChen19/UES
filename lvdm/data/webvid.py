...
class WebVid(Dataset):
    ...
    def _load_metadata(self):
        with open(self.meta_path, 'r') as f:
            metadata = json.load(f)  # 加载json文件

        print(f'>>> {len(metadata)} data samples loaded.')
        
        if self.subsample is not None:
            metadata = random.sample(metadata, self.subsample)  # 进行采样

        metadata_df = pd.DataFrame(metadata)
        
        metadata_df['caption'] = metadata_df['cap'].apply(lambda x: x[0]) 
    ...
    def __getitem__(self, index):
        ...
        while True:
            ...
            try:
                ...
                print(f"Get frames failed! Path = {video_path}; [max_ind vs frame_total: {max(frame_indices)} / {frame_num}]. Error: {e}")
                index += 1
                continue
