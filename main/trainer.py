...
if __name__ == "__main__":
    ...
    if args.train:
        try:
                ...
                if trainer_kwargs['precision'] == 16:
                    with torch.cuda.amp.autocast():
                        if trainer.global_step == 250:
                            logger.info(f"Current input data shape: {data.shape}")
                            logger.info(f"Current input data: {data}")
            ...
            logger.error(f"Error during training at step {trainer.global_step}: {e}")
            raise
