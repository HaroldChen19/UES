...
def load_data_prompts(data_dir, video_size=(256, 256), video_frames=16, interp=False):
    ...
    ## Load video folder names instead of individual frames
    video_folders = [f for f in os.listdir(data_dir) if os.path.isdir(os.path.join(data_dir, f))]
    video_folders.sort()  # Sort to ensure the order matches the prompts
    ...
    for idx in range(n_samples):
        ...
        folder_path = os.path.join(data_dir, video_folder)
        
        # Get all frame images in the folder
        frame_files = get_filelist(folder_path, ['jpg', 'png', 'jpeg', 'JPEG', 'PNG'])
        frame_files.sort()  # Sort frames to maintain the correct sequence
        ...
        # If the number of frames is more than `video_frames`, sample `video_frames` frames evenly
        if len(frame_tensors) > video_frames:
            step = len(frame_tensors) // video_frames
            frame_tensors = [frame_tensors[i] for i in range(0, len(frame_tensors), step)][:video_frames]
        
        # If the number of frames is less than `video_frames`, pad by repeating frames
        if len(frame_tensors) < video_frames:
            frame_tensors = frame_tensors * (video_frames // len(frame_tensors)) + frame_tensors[:video_frames % len(frame_tensors)]
        
        # Concatenate frame tensors along the temporal dimension (time)
        frame_tensor = torch.cat(frame_tensors, dim=1)  
        
        data_list.append(frame_tensor)
        filename_list.append(video_folder)
...
def image_guided_synthesis(model, prompts, videos, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1., \
    ...
    if model.model.conditioning_key == 'hybrid':
        ...
        cond["c_concat"] = [img_cat_cond.zero_()] # b c 1 h w
    ...
    if unconditional_guidance_scale != 1.0:
        ...
        uc_img_emb = torch.zeros_like(videos[:,:,0])
        uc_img_emb = uc_img_emb.repeat(t, 1, 1, 1)
        uc_img_emb = model.embedder(uc_img_emb) ## b l c
        uc_img_emb = model.image_proj_model(uc_img_emb)
        uc_img_emb = uc_img_emb.view(b, t, 256, 1024)

        uc_img_emb = uc_img_emb.mean(dim=1)
    ...
    if multiple_cond_cfg and cfg_img != 1.0:
        ...
        if model.model.conditioning_key == 'hybrid':
            uc_2["c_concat"] = [img_cat_cond.zero_()]
...
def run_inference(args, gpu_num, gpu_no):
    ...
    ######LoRA
    model.unet_lora_params_temporal, model.unet_negation_temporal, model.unet_lora_params_spatial, model.unet_negation_spatial = model.model.LoRA_Module()
    ######
...
if __name__ == '__main__':
    ...
    print("@UES cond-Inference: %s"%now)
    parser = get_parser()
