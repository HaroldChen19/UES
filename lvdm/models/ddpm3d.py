...
def create_optimizer_params(model_list, lr):
    ...
    for optim in model_list:
        ...
        if is_lora and condition and isinstance(model, list):
            params = list(itertools.chain(*model))  
            optimizer_params.extend(params)  
            continue
...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        # unet_lora_params_temporal, unet_negation_temporal = self.model.LoRA_Module()
        # self.unet_lora_params_temporal = unet_lora_params_temporal
        # self.unet_negation_temporal = unet_negation_temporal
...
class LatentDiffusion(DDPM):
    ...
    def apply_model(self, x_noisy, t, cond, **kwargs):
        ...
        target_replace_module = {"TemporalTransformer", "SpatialTransformer"}
        loras = extract_lora_child_module(model, target_replace_module=target_replace_module)
        scale_loras(loras, 1.0)

        x_recon = model(x_noisy, t, **cond, **kwargs)
    ...
    def p_losses(self, x_start, cond, t, noise=None, **kwargs):
        ...
        # loss_simple = create_ad_temporal_loss(model_output, loss_simple, target)
...
class LatentVisualDiffusion(LatentDiffusion):
    ...
    def _init_embedder(self, config, freeze=True):
        ...
        if freeze:
            ...
            for param in self.embedder.parameters():
                param.requires_grad = False
            for n, p in self.embedder.named_parameters():
                if 'adapter' in n:
    ...
    def get_batch_input(self, batch, random_uncond, return_first_stage_outputs=False, return_original_cond=False, return_fs=False, return_cond_frame=False, return_original_input=False, **kwargs):
        ...
        input_mask = 1 - rearrange((random_num >= self.uncond_prob).float() * (random_num < 3 * self.uncond_prob).float(), "n -> n 1 1 1 1")
        ...
        b, c, t, h, w = x.shape
        input_mask = input_mask.expand(-1, -1, t, -1, -1)
        ...
        img_emb = img_emb.view(b, t, 256, 1024)
        ...
        if self.model.conditioning_key == 'hybrid':
            ...
            cond["c_concat"] = [img_cat_cond.zero_()] # b c t h w
    ...
    def configure_optimizers(self):
        ...
        # print("here_3")
        with open('requires_grad_params.txt', 'w') as f:
            for name, param in self.model.named_parameters():
                if param.requires_grad:
                    f.write(f"Parameter: {name}, Shape: {param.shape}\n")

        import itertools
        lr = self.learning_rate

        unet_lora_params_temporal = list(itertools.chain(*[list(p) if isinstance(p, (list, tuple)) else list(p) for p in self.unet_lora_params_temporal]))
        unet_lora_params_spatial = list(itertools.chain(*[list(p) if isinstance(p, (list, tuple)) else list(p) for p in self.unet_lora_params_spatial]))

        print("unet_lora_params_temporal types:", [type(p) for p in unet_lora_params_temporal])

        params = unet_lora_params_temporal
        params.extend(unet_lora_params_spatial)

        mainlogger.info(f"@Training [{len(params)}] LoRA Paramters.")

        params_adapter = [p for p in self.embedder.parameters() if p.requires_grad == True]
        mainlogger.info(f"@Training [{len(params_adapter)}] Paramters for Adapters.")
        params.extend(params_adapter)
        # print("Adapter:", type(params_adapter)
        # print("params_adapter types:", [type(p) for p in params_adapter])
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def LoRA_Module(self):
        self.diffusion_model.requires_grad_(False)
        target_temporal_modules=["TemporalTransformer"]
        target_spatial_modules=["SpatialTransformer"]
        ...
        # print("lora_manager_temporal.unet_replace_modules:", lora_manager_temporal.unet_replace_modules)
        lora_path = create_save_paths("./output_ft_test_9_13")
        unet_lora_params_temporal, unet_negation_temporal = lora_manager_temporal.add_lora_to_model(
            True, self.diffusion_model, lora_manager_temporal.unet_replace_modules, 0,
            lora_path + '/temporal/', r=32)
        unet_lora_params_spatial, unet_negation_spatial = lora_manager_spatial.add_lora_to_model(
            True, self.diffusion_model, lora_manager_spatial.unet_replace_modules, 0.1,
            lora_path + '/spatial/', r=32)
        
        # self.unet_lora_params_temporal = unet_lora_params_temporal
        # self.unet_negation_temporal = unet_negation_temporal
        
        return unet_lora_params_temporal, unet_negation_temporal, unet_lora_params_spatial, unet_negation_spatial
